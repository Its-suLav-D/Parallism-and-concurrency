...
class Board():
    ...
    def dfs(self, row, col, word, index, direction):
        if index == len(word):
            return True
        ...
        if self.dfs(next_row, next_col, word, index + 1, direction):
            self.highlight(row, col)
            return True
        return False
    ...
    def find_word_with_dfs(self, word):
        print(f'Finding {word}...')
        for row in range(self.size):
            for col in range(self.size):
                for direction in self.directions:
                    if self.dfs(row, col, word, 0, direction):
                        return True
        return False

def worker(args):
    board, word = args
    found = board.find_word_with_dfs(word)
    return word, found, board.highlighting
